...
async function fetchImageInline(url: string): Promise<InlineImage> {
  ...
  const headerType = (res.headers['content-type'] as string | undefined)?.split(';')[0]?.trim();
  let mimeType = headerType && headerType.startsWith('image/') ? headerType : '';
  if (!mimeType) {
...
const styles = {
  ...
  martoon: ['https://withjoy.blob.core.windows.net/paper-ai/style-martoon-01.png'],
};
...
export async function generateStylizedImage(styleId: StyleID, subjectUrl: string): Promise<Buffer> {
    ...
    parts: [
      {
        text: [
          ...
          'Leave generous white margins around the artwork so the subjects never touch the edges of the frame: 20-30% white margin on the left and right, 35-50% white margin at the top, and 15-25% white margin at the bottom.',
          'The artwork should artistically transition to white at all four edges within those margins.',
  ...
  const partData = response.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data;
  if (!partData) {
    const finishReason = response.candidates?.[0]?.finishReason;
    throw new GenerateStylizedImageError(finishReason ?? 'UNKNOWN');
  }
  return Buffer.from(partData, 'base64');
}
