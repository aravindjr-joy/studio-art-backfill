...
interface EventRecord {
  ...
  savedToDisk: string | null;
  filestackUrl: string | null;
...
function parseLogs(logLines: string[]): EventRecord[] {
  ...
  for (const line of logLines) {
    ...
    switch (key) {
      ...
      case 'saved-to-disk': current.savedToDisk = value; break;
      case 'filestack-url': current.filestackUrl = value; break;
...
function parseRunSummary(text: string): { header: RunHeader; events: EventRecord[] } {
...
function namesLine(event: EventRecord): string {
...
function eventCardHtml(event: EventRecord): string {
...
function buildHtml(header: RunHeader, events: EventRecord[]): string {
  ...
  const summaryRows = [
    ...
    .map(
      ([k, v]) =>
        `<div class="row"><dt>${escapeHtml(k!)}</dt><dd>${escapeHtml(v!)}</dd></div>`,
    )
    .join('');
...
<body>
  <div class="container">
    ...
    ${cards || '<p class="muted">No successful events to display.</p>'}
  </div>
</body>
</html>`;
}
...
function main(): void {
  ...
  const html = buildHtml(header, events);
